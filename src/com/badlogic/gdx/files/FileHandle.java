...
public class FileHandle
{
	...
	protected FileHandle(String fileName, FileType type, boolean isResource)
	{
		this.type = type;
		file = new File(fileName);
	...
	protected FileHandle(File file, FileType type, boolean isResource)
	{
		this.file = file;
		this.type = type;
		this.isResource = isResource;
	...
	public InputStream read()
	{
		if (isResource)
		{
			try
			{
				InputStream inputStream = new ResourceLoader().loadResourceStream(file.getPath());
				if (inputStream == null) throw new GdxRuntimeException("InputStream for " + file + " was null!!!");
				return inputStream;
			} catch (Exception e)
			{
				if (file().isDirectory())
					throw new GdxRuntimeException("Cannot open a stream to a directory: " + file + " (" + type + ")", e);
